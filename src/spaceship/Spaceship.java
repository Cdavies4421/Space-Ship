...
// draw border
        ...
        for(int i = 0; i < Missile.current; i++){
            if(missiles[i].active)
            {
                g.setColor(Color.red);
                drawCircle(getX(missiles[i].xpos),getYNormal(missiles[i].ypos),0.0,0.5,0.5);
            }
        }
...
/////////////////////////////////////////////////////////////////////////
    public void animate() {
        ...
        if(rocketXSpeed >= maxSpeed)
        {
            rocketXSpeed = maxSpeed;
        }
        if(rocketXSpeed <= -maxSpeed)
        {
            rocketXSpeed = -maxSpeed;
        ...
        for(int i = 0; i < starXPos.length; i++)
        {
            if(starXPos[i]-15 < rocketXPos && 
               starXPos[i]+15 > rocketXPos &&
               starYPos[i]-15 < rocketYPos &&
               starYPos[i]+15 > rocketYPos )
            {
                if(i != whichStarHit){
        ...
        for(int i = 0; i < Missile.numMissiles; i++)
        {
            for(int p = 0; p < starXPos.length-1; p++){
                if(missiles[i].xpos-15 < starXPos[p] && 
                   missiles[i].xpos+15 > starXPos[p] &&
                   missiles[i].ypos-15 < starYPos[p] &&
                   missiles[i].ypos+15 > starYPos[p] )
                {
                    missiles[i].active = false;
                    if(rocketRight){
                        starXPos[p] = getWidth2();
                        starYPos[p] = (int)(Math.random() * getHeight2());
                    }
                    else{
                        starXPos[p] = 0;
                        starYPos[p] = (int)(Math.random() * getHeight2());
...
/////////////////////////////////////////////////////////////////////////
    ...
    public void readFile() {
        try {
            String inputfile = "game-info.txt";
...
class Missile{
    ...
    public static final int numMissiles = 6;
